...
class ReportController:
    ...
    def get_tournament_players_sorted(self, tournament_id):
        """
        Charge et retourne la liste des joueurs pour un
         tournoi spécifié, triée par leur score en ordre décroissant.

        Args:
            tournament_id (int): L'ID du tournoi dont
            on souhaite obtenir la liste des joueurs.

        Returns:
            list: Liste des joueurs du tournoi triés
            par score en ordre décroissant.
        """
        report = Report()
        tournaments = report.load_tournaments()
